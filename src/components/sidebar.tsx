...
import { BackpackIcon, ChevronRightIcon, TrashIcon } from '@radix-ui/react-icons'
import { useAppDispatch, useAppSelector } from '@/store'
import { addRequestTab } from '@/store/actions'
import { buildSidebarStructure, createNewFolder, createNewRequest, deleteCollection, setActiveCollection, setActiveFolder, updateStoreFromCollection } from '@/lib/utils'
        ...
        <div className='p-4 pr-0 flex flex-col justify-between text-sm h-screen'>
            <div className='sticky top-0'>
                <svg width="75" height="28" viewBox="0 0 89 28" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <path className="fill-black dark:fill-white" d="M44.5103 24.278C47.3903 24.278 49.9103 22.118 49.9103 18.086C49.9103 14.09 47.3903 11.93 44.5103 11.93C41.6303 11.93 39.1103 14.09 39.1103 18.086C39.1103 22.118 41.6303 24.278 44.5103 24.278ZM44.5103 8.906C49.6943 8.906 53.3663 12.83 53.3663 18.086C53.3663 23.378 49.6943 27.302 44.5103 27.302C39.3263 27.302 35.6543 23.378 35.6543 18.086C35.6543 12.83 39.3263 8.906 44.5103 8.906Z" />
                    <path className="fill-black dark:fill-white" d="M66.7721 24.674C65.8361 26.474 63.6761 27.302 61.6601 27.302C57.6281 27.302 55.1801 24.278 55.1801 20.354V9.446H58.5641V19.85C58.5641 22.226 59.6441 24.314 62.4881 24.314C65.2241 24.314 66.5921 22.514 66.5921 19.886V9.446H69.9761V23.558C69.9761 24.926 70.0841 26.15 70.1561 26.762H66.9161C66.8441 26.366 66.7721 25.394 66.7721 24.674Z" />
                    <path className="fill-black dark:fill-white" d="M78.1268 3.974V9.446H81.8708V12.506H78.1268V21.506C78.1268 23.09 78.7747 23.882 80.5387 23.882C80.9708 23.882 81.5828 23.81 81.8708 23.738V26.618C81.5828 26.726 80.7188 26.942 79.5667 26.942C76.6147 26.942 74.7428 25.142 74.7428 22.01V12.506H71.4307V9.446H72.3668C74.2388 9.446 75.0308 8.294 75.0308 6.782V3.974H78.1268Z" />
                    <path className="fill-black dark:fill-white" d="M83.8676 24.458C83.8676 23.054 84.9476 21.938 86.3516 21.938C87.7556 21.938 88.8716 23.054 88.8716 24.458C88.8716 25.862 87.7556 26.942 86.3516 26.942C84.9476 26.942 83.8676 25.862 83.8676 24.458Z" fill="#0085FF" />
                </svg>
            </div>
...
function SidebarCollectionComponent(props: SidebarCollection) {
    ...
    const [deleteCollectionDialogOpen, setDeleteCollectionDialogOopen] = React.useState(false)

    return (
        <>
            <div>
                <div className={`py-1 px-2 ${activeCollectionId === props.id ? 'dark:bg-neutral-800 bg-neutral-100' : ''} w-full text-left flex items-center justify-between`}>
                    <button className='flex gap-3 items-center w-full'
                        onClick={() => {
                            setActiveCollection(dispatch, props.id)
                            setOpen(!open)
                        }}>
                        <ChevronRightIcon className={`transform duration-300 ${open ? 'rotate-90' : 'rotate-0'}`} />
                        {props.name}
                    </button>
                    ...
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent>
                            ...
                            <DropdownMenuItem onClick={() => createNewFolder(dispatch, props.id, "")}>
                                New Folder
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => createNewRequest(dispatch, props.id, "")}>
                                New Request
                            </DropdownMenuItem>
            ...
            <Dialog open={deleteCollectionDialogOpen} onOpenChange={setDeleteCollectionDialogOopen}>
