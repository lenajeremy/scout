...
function RequestTab(props: { id: string, name: string }) {
    ...
    return (
        <>
            <button className={`flex text-left gap-6 items-center justify-between px-2 py-1.5 duration-200 rounded-lg ${activeTabId === props.id ? 'p-1 text-neutral-500 bg-secondary text-secondary-foreground' : 'bg-muted'}`} onClick={() => dispatch(addRequestTab(props))}>
